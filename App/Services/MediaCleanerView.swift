...
// UI to run privacy-first scan (metadata only) + submit summary to Core.
// Includes Slim Mode toggle (estimation only).
...
struct MediaCleanerView: View {
    ...
    @State private var slimModeEnabled: Bool = false
        ...
        NavigationView {
            Form {
                ...
                Section(header: Text("Options")) {
                    Toggle("Slim Mode (estimate savings)", isOn: $slimModeEnabled)
                }
    ...
    private func runScan() async {
        ...
        do {
            let result = try await MediaCleanerService.shared.runCleanupFlow(
                ...
                deviceId: deviceId,
                mode: mode,
                slimModeEnabled: slimModeEnabled,
                onProgress: { p in
